    ...
    try {
        ...
        if(users.verified){
            ...
            res.render("changepassword", {
                ...
                check:"1",
                _id: id,role:req.session.role,idcheck:req.session._idcheck
              });
        }
        else{
            if (!existingToken) {
                ...
                res.render("changepassword", {
                    ...
                    type:req.session.type,
                    footer: 1,
                    _id: id,role:users.role,idcheck:req.session._idcheck
                  });
            }
        }   
      } catch (error) {
...
const watchsales=async(req,res)=>{
    ...
    const customername="";
    var result=[];
    const customer = await Customers.find({}).lean();
    for(const _customer of customer){
        for(const user of _customer.purchaseHistory){
            if(JSON.stringify(user.userId)===JSON.stringify(id)){
                const _user={};
                _user.phoneNumber=_customer.phoneNumber;
                _user.fullName=_customer.fullName;
                _user.address=_customer.address;
                _user.changereturn=user.changeReturned;
                _user.amountPaid=user.amountPaid;
                _user.date=user.orderDate;
                _user.totalprice=user.totalAmount;
                result.push(_user);
            }
        }
    }
    console.log(result);
    res.render("SalesInformation",{users:result,username:req.session.username,footer:1,_id:req.session.remember,role:req.session.role});
}
