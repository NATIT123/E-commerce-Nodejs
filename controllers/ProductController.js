...
const getproduct=async(req,res)=>{
    ...
    const products=await Products.find({}).skip((page-1)*booksPerPage).limit(booksPerPage);
    ...
    for(let i=1;i<=totalPage;i++){
        ...
        if(product){
            res.render("product",{products:mutipleMongooseToObject(products),
                username:req.session.username,footer:1,_id: req.session.remember,footer:1,totalPage:Number(totalPage),array:array,
                currentpage:Number(page),message:req.session.product,type:req.session.productype,role:req.session.role})   
        }
    // else{
    //     if(existingToken){
...
const getcreate=(req,res)=>{
    ...
    res.render("addproduct",{username:req.session.username,footer:1,message:req.session.productadd,type:req.session.producttypeadd,role:req.session.role});
};
...
const getupdate=(req,res)=>{
    ...
    Products.findById(id).then(product=>{
        if(product){
            ...
            title:"Update Product",message:req.session.productupdate,type:req.session.producttyeupdate,role:req.session.role
            })
        }
        else{
            res.redirect("/products");
...
const deleteproduct=async(req,res,next)=>{
    var id=mongoose.Types.ObjectId.isValid(req.params.id) ? new mongoose.Types.ObjectId(req.params.id) : req.params.id;
    const _product=await Products.findOne({_id:id});
    if(_product.status){
        req.session.product="Product has been sold";
