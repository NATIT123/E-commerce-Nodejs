...
exports.listCustomers = async (req, res) => {
  res.render("listCustomers",{username:req.session.username,footer:1,_id:req.session.remember,role:req.session.role});
};
...
exports.profile = async (req, res) => {
  ...
  try {
    ...
    return res.render("customerInfor", { customer,username:req.session.username,footer:1,_id:req.session.remember,role:req.session.role });
  } catch (error) {
    console.error(error);
...
exports.createOrder = async (req, res) => {
  return res.render("transaction",{username:req.session.username,footer:1,_id:req.session.remember,role:req.session.role});
};

exports.createPurchase = async (req, res) => {
  return res.render("checkOut",{username:req.session.username,footer:1,_id:req.session.remember,role:req.session.role});
};
...
exports.searchProduct = async (req, res) => {
  ...
  let product = await Products.findOne({ barcode: barcode });
...
exports.storePurchase = async (req, res) => {
  ...
  if (customer) {
    ...
    for(const _p of purchase){
        console.log(_p.productId);
        const _product=await Products.findOne({_id:_p.productId});
        if(!_product.status){
          await Products.findByIdAndUpdate(_p.productId,{status:true});
        }
    }
    // store purchase
